...
export function parseExpression(expression) {
    ...
    switch (expression.type) {
        ...
        case '<=':
        case '>=':
            return `${expression.left} ${expression.type} ${typeof expression.right === 'string'
                ? `'${expression.right}'`
                : expression.right}`;
        case 'between':
        case 'not between':
            return `${expression.field} ${expression.type.toUpperCase()} ${typeof expression.left === 'string'
                ? `'${expression.left}'`
                : expression.left} AND ${typeof expression.right === 'string'
                ? `'${expression.right}'`
                : expression.right}`;
        case 'is null':
        case 'is not null':
        ...
        case 'coalesce':
            return `COALESCE(${expression.values
                .map((field) => parseExpression(field))
                .join(', ')})`;
        case 'in':
        case 'not in':
            ...
            else if (typeof expression.set === 'object' &&
        ...
        case 'gis_mapping_lon':
            return `gis_discrete_trans_scale_long_epsg_4326_900913 (${expression.domainStart}::float, ${expression.domainEnd}::float, ${expression.field}, ${Math.floor(expression.range)})`;
        case 'gis_mapping_lat':
            return `gis_discrete_trans_scale_lat_epsg_4326_900913 (${expression.domainStart}::float, ${expression.domainEnd}::float, ${expression.field}, ${Math.floor(expression.range)})`;
        case 'gis_discrete_trans_scale_w':
            return `gis_discrete_trans_scale(${expression.domain[0]}, ${expression.domain[1]}, 0, ${expression.width - 1}, ${expression.field}::float)`;
